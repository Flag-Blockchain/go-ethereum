...
func (c *Console) initExtensions() error {
	...
	aliases := map[string]struct{}{"flag": {}, "personal": {}}
	for api := range apis {
		if api == "web3" {
			continue
		}
...
func (c *Console) Welcome() {
	...
	if res, err := c.jsre.Run(`
		...
		try {
			message += "coinbase: " + flag.coinbase + "\n";
		} catch (err) {}
		message += "at block: " + flag.blockNumber + " (" + new Date(1000 * flag.getBlock(flag.blockNumber).timestamp) + ")\n";
		try {
